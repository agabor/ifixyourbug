<template>
  <div class="col-md-12 pe-2">
    <div class="form-check form-switch">
      <input class="form-check-input" type="checkbox" id="specific-op-system-input" :value="isSpecificOpSystem" @change="updateIsSpecificOpSystem" :checked="isSpecificOpSystem" :disabled="!editable">
  ...
  <div class="col-md-12 d-flex pe-2" v-if="isSpecificOpSystem">
    <div class="form-check me-3" v-for="n in optionCount" :key="n">
      <input class="form-check-input" type="radio" name="osRadio" :id="`osRadio${n}`" :value="$t(`operatingSystem.option${n}`)" v-model="os" @change="$emit('update:operatingSystem', os)" :checked="$t(`operatingSystem.option${n}`) == os" :disabled="!editable">
  ...
  <div class="col-md-12 pe-2 mb-3" v-if="isSpecificOpSystem">
    <div class="form-group mb-0">
      ...
      <input id="op-system-name-input" class="form-control" :placeholder="$t('operatingSystem.version')" type="text" :value="version" @input="$emit('update:version', $event.target.value)" :disabled="!editable">
    </div>
  </div>
</template>
